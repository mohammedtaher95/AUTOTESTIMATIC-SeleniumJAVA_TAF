...
node {
    ...
    stage('Run Tests') {
        ...
        withEnv(["MVN_HOME=$mvnHome"]) {
            if (isUnix()) {
                sh '"$MVN_HOME/bin/mvn" -Dmaven.test.failure.ignore clean deploy -s settings.xml -X'
            } else {
                bat(/"%MVN_HOME%\bin\mvn" -Dmaven.test.failure.ignore clean deploy -s settings.xml -X/)
            }
        }
    }
